...
// ("Warp stripes"). Zero dependencies: one WebGL canvas that fills its
// parent. Drop it behind your content with a positioned wrapper.
//
// Converted from the generator's TypeScript to plain JSX for this project:
// the "use client" directive is Next-only and the type annotations are gone,
...
const UNIFORMS = {
  colors: [
    [0.050980392156862744, 0.10588235294117647, 0.058823529411764705],
    [0.24313725490196078, 0.5568627450980392, 0.2549019607843137],
    [0.7803921568627451, 0.9568627450980393, 0.39215686274509803],
    [1, 0.9921568627450981, 0.8823529411764706],
    [1, 0.9921568627450981, 0.8823529411764706],
    [1, 0.9921568627450981, 0.8823529411764706],
    [1, 0.9921568627450981, 0.8823529411764706],
    [1, 0.9921568627450981, 0.8823529411764706],
...
// seed and phase are the two knobs a second copy of this needs to not read
// as the same animation playing twice. Both default to the generator's own
// recipe, so an instance that passes neither is untouched.
//
// seed moves the noise field the warp is drawn from. phase is a head start
// in seconds — the stripe pattern's own term is u_time * 0.5, and u_time is
// wall seconds times this recipe's -0.195 time scale, so the pattern comes
// back round about every 64s (2pi / (0.195 * 0.5)); a phase a third of that
// apart lands a copy at a visibly different point in it.
function ShaderBackground({ className, seed = UNIFORMS.seed, phase = 0 }) {
  ...
  useEffect(() => {
    ...
    function render(now) {
      ...
      gl.uniform4f(
        ...
        width,
        height,
        ((now - start) / 1000 + phase) * UNIFORMS.timeScale,
        UNIFORMS.colorCount,
      );
      gl.uniform4f(
        uni.space,
    ...
    // Primitives, so this only ever re-runs if a caller actually changes
    // one — which none of them do after mount.
  }, [seed, phase]);
